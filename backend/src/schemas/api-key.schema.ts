...
// =============================================================================
// Employee Push Sync Schemas (Desktop → Server)
// =============================================================================

/**
 * Valid employee roles for push sync
 * Desktop can only push CASHIER records (managers are created via admin UI)
 */
export const EMPLOYEE_PUSH_ROLES = ["CASHIER"] as const;

/**
 * Single employee record for push sync
 * Desktop sends cashiers to server for cloud persistence
 *
 * Security Controls:
 * - API-001: VALIDATION - All fields validated with Zod
 * - SEC-001: PASSWORD_HASHING - PIN is hashed on desktop, sent as pin_hash
 * - DB-006: TENANT_ISOLATION - store_id derived from API key session
 */
export const employeeSyncPushRecordSchema = z.object({
  /** UUID for the cashier (generated by desktop) */
  ...
  /** Role (only CASHIER supported for push) */
  role: z.enum(EMPLOYEE_PUSH_ROLES, {
    message: "Only CASHIER role can be pushed from desktop",
  }),

  /** bcrypt-hashed PIN for authentication (hashed on desktop) */
  pin_hash: z
    .string()
    .min(50, "pin_hash must be a valid bcrypt hash")
    .max(100, "pin_hash exceeds maximum length")
    .regex(/^\$2[aby]?\$\d{2}\$/, "pin_hash must be a valid bcrypt hash"),

  /** 4-digit employee code (optional, auto-generated if not provided) */
  employee_code: z
    .string()
    .regex(/^\d{4}$/, "employee_code must be exactly 4 numeric digits")
    .optional(),
  ...
  hired_on: z
    .string()
    .datetime("hired_on must be a valid ISO 8601 datetime")
    .optional(),

  /** Termination date (optional) */
  termination_date: z
    .string()
    .datetime("termination_date must be a valid ISO 8601 datetime")
    .optional()
    .nullable(),

  /**
   * User ID who created this employee (optional)
   * If not provided, uses the store's store_login_user_id
   */
 ...
 * Schema for pushing employee records from desktop to server
 *
 * Supports batch operations for efficiency:
 * - Create new cashiers
 * - Update existing cashiers (idempotent)
 *
 * Security Controls:
 * - API-001: VALIDATION - All fields validated with Zod
 * - SEC-001: PASSWORD_HASHING - PIN hashes accepted, not plaintext
 * - DB-006: TENANT_ISOLATION - store_id from session validation
 */
export const employeeSyncPushSchema = z.object({
  /** Sync session ID from /api/v1/sync/start */
  session_id: z.string().uuid("session_id must be a valid UUID"),

  /** Array of employee records to sync */
  employees: z
    .array(employeeSyncPushRecordSchema)
    .min(1, "At least one employee is required")
    .max(100, "Cannot push more than 100 employees at once"),
});

export type EmployeeSyncPushRecord = z.infer<
  typeof employeeSyncPushRecordSchema
>;
export type EmployeeSyncPushInput = z.infer<typeof employeeSyncPushSchema>;
