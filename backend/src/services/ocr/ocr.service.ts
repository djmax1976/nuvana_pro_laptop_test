...
export class OCRService {
  ...
  /**
   * Validate extracted report date against expected business date.
   */
  private validateReportDate(
    ...
    try {
      // Parse report date (MM/DD/YYYY format)
      const reportParts = reportDate.match(/(\d{1,2})\/(\d{1,2})\/(\d{4})/);
      if (!reportParts) {
        return {
          isValid: false,
          expectedDate,
          reportDate,
          errorMessage: `Could not parse report date: ${reportDate}`,
        };
      }

      const reportMonth = parseInt(reportParts[1], 10);
      const reportDay = parseInt(reportParts[2], 10);
      const reportYear = parseInt(reportParts[3], 10);
      const reportDateObj = new Date(reportYear, reportMonth - 1, reportDay);

      // Parse expected date (YYYY-MM-DD format)
      const expectedParts = expectedDate.match(/(\d{4})-(\d{2})-(\d{2})/);
      if (!expectedParts) {
        return {
          isValid: false,
          expectedDate,
          reportDate,
          errorMessage: `Invalid expected date format: ${expectedDate}`,
        };
      }

      const expectedYear = parseInt(expectedParts[1], 10);
      const expectedMonth = parseInt(expectedParts[2], 10);
      const expectedDay = parseInt(expectedParts[3], 10);
      ...
      // Compare dates
      const isValid = reportDateObj.getTime() === expectedDateObj.getTime();

      if (!isValid) {
        const formattedExpected = `${expectedMonth}/${expectedDay}/${expectedYear}`;
        return {
          isValid: false,
          expectedDate,
          reportDate,
          errorMessage: `Report date ${reportDate} does not match expected date ${formattedExpected}. Please ensure you are scanning the correct day's report.`,
        };
      }
      ...
      return {
        ...
        expectedDate,
        reportDate,
        errorMessage: `Date validation error: ${error instanceof Error ? error.message : String(error)}`,
      };
    }
  }
