...
test.describe.skip(
  "1.5-API-001: OAuth Callback Endpoint [DISABLED - OAuth not supported]",
  () => {
...
test.describe.skip(
  "1.5-API-002: Token Validation Middleware [DISABLED - OAuth not supported]",
  () => {
    // Clean up all users before each test to ensure isolation
    test.beforeEach(async ({ prismaClient }) => {
      await prismaClient.user.deleteMany({});
...
test.describe.skip(
  "1.5-API-003: User Service - getUserOrCreate [DISABLED - OAuth not supported]",
  () => {
    // Clean up all users before each test to ensure isolation
    test.beforeEach(async ({ prismaClient }) => {
      await prismaClient.user.deleteMany({});
