...
describe.skip("10-4-COMPONENT: Manual Entry Mode", () => {
  ...
  it("10-4-COMPONENT-013: should still validate range in manual mode", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByTestId("ending-number-error-bin-1"),
      ).toBeInTheDocument();
      expect(
  ...
  it("10-4-COMPONENT-SEC-003: should prevent XSS in manual entry mode", async () => {
    // GIVEN: LotteryShiftClosingPage component
    const { LotteryShiftClosingPage } =
      await import("@/components/shift-closing/LotteryShiftClosingPage");

    // WHEN: Component renders with XSS attempt in user name
    // (This would be tested if user name comes from API)
    renderWithProviders(<LotteryShiftClosingPage />);
  ...
  it("10-4-COMPONENT-EDGE-010: should handle rapid mode transitions", async () => {
    // GIVEN: LotteryShiftClosingPage component
    const { LotteryShiftClosingPage } =
      await import("@/components/shift-closing/LotteryShiftClosingPage");
    const user = userEvent.setup();
    ...
    // WHEN: Rapidly clicking manual entry button multiple times
    await user.click(manualEntryButton);
    await user.click(manualEntryButton);
    await user.click(manualEntryButton);

    // THEN: Component handles rapid clicks gracefully
    // (Should not open multiple modals or cause state issues)
    await waitFor(() => {
      const modals = screen.queryAllByTestId("manual-entry-auth-modal");
      expect(modals.length).toBeLessThanOrEqual(1); // At most one modal
    });
  });

  it("10-4-COMPONENT-EDGE-011: should handle navigation during manual entry mode", async () => {
    // GIVEN: LotteryShiftClosingPage with active manual entry mode
    const { LotteryShiftClosingPage } =
      await import("@/components/shift-closing/LotteryShiftClosingPage");
  ...
  it("10-4-COMPONENT-ASSERT-003: should have proper test IDs for all interactive elements", async () => {
    // GIVEN: LotteryShiftClosingPage component
    const { LotteryShiftClosingPage } =
      await import("@/components/shift-closing/LotteryShiftClosingPage");
