...
test.describe("10-5-INTEGRATION: Add Bin Flow", () => {
  test("10-5-INTEGRATION-001: [P0] Full add bin flow creates all records in transaction", async ({
    ...
    expect(binCountAfter, "Bin count should increase by 1").toBe(
      binCountBefore + 1,
    );
  ...
  test("10-5-INTEGRATION-002: [P0] New bin appears in closing page after creation", async ({
    ...
    expect(binsAfterResponse.status(), "Bins list request should succeed").toBe(
      200,
    );
  ...
  test("10-5-INTEGRATION-003: [P0] Transaction rollback prevents partial data on error", async ({
    ...
    expect(
      shiftOpeningCountAfter,
      "Shift opening count should not change",
    ).toBe(shiftOpeningCountBefore);
    ...
    expect(
      unchangedPack?.current_bin_id,
      "Pack current_bin_id should be null",
    ).toBeNull();
    expect(
      unchangedPack?.activated_at,
      "Pack activated_at should be null",
    ).toBeNull();
    expect(
      unchangedPack?.activated_by,
      "Pack activated_by should be null",
    ).toBeNull();
    expect(
      unchangedPack?.activated_shift_id,
      "Pack activated_shift_id should be null",
    ).toBeNull();
  });
  ...
  test("10-5-INTEGRATION-ENH-001: [P0] Full flow should maintain referential integrity", async ({
    storeManagerApiRequest,
    storeManagerUser,
    prismaClient,
  }) => {
    ...
    // AND: All foreign key relationships are valid
    const bin = await prismaClient.lotteryBin.findUnique({
      where: { bin_id: createdBin.bin_id },
      include: {
        store: true,
    ...
    expect(bin?.store_id, "Bin should reference valid store").toBe(
      storeManagerUser.store_id,
    );
    expect(bin?.store, "Bin should have store relation").toBeDefined();
    ...
    expect(
      updatedPack?.current_bin_id,
      "Pack should reference created bin",
    ).toBe(createdBin.bin_id);
    expect(updatedPack?.bin, "Pack should have bin relation").toBeDefined();
    expect(updatedPack?.bin?.bin_id, "Pack bin relation should match").toBe(
      createdBin.bin_id,
    );
    expect(updatedPack?.game, "Pack should have game relation").toBeDefined();
    expect(
      updatedPack?.activatedByUser,
      "Pack should have activatedByUser relation",
    ).toBeDefined();
    expect(
      updatedPack?.activatedByUser?.user_id,
      "Pack activatedByUser should match",
    ).toBe(storeManagerUser.user_id);
    expect(
      updatedPack?.activatedShift,
      "Pack should have activatedShift relation",
    ).toBeDefined();
    expect(
      updatedPack?.activatedShift?.shift_id,
      "Pack activatedShift should match",
    ).toBe(shift.shift_id);

    // AND: LotteryShiftOpening references are valid
    const shiftOpening = await prismaClient.lotteryShiftOpening.findFirst({
      where: {
        shift_id: shift.shift_id,
        pack_id: pack.pack_id,
  ...
  test("10-5-INTEGRATION-ENH-002: [P0] Full flow should set all timestamp fields correctly", async ({
    ...
    // AND: All timestamp fields are set correctly
    const bin = await prismaClient.lotteryBin.findUnique({
      where: { bin_id: createdBin.bin_id },
    });
    expect(bin?.created_at, "Bin created_at should be set").toBeDefined();
    expect(bin?.created_at, "Bin created_at should be a Date").toBeInstanceOf(
      Date,
    );
    expect(
      bin?.created_at!.getTime(),
      "Bin created_at should be after beforeTime",
    ).toBeGreaterThanOrEqual(beforeTime.getTime());
    expect(
      bin?.created_at!.getTime(),
      "Bin created_at should be before afterTime",
    ...
    expect(
      updatedPack?.activated_at,
      "Pack activated_at should be a Date",
    ).toBeInstanceOf(Date);
    expect(
      updatedPack?.activated_at!.getTime(),
      "Pack activated_at should be after beforeTime",
    ).toBeGreaterThanOrEqual(beforeTime.getTime());
    expect(
      updatedPack?.activated_at!.getTime(),
      "Pack activated_at should be before afterTime",
    ).toBeLessThanOrEqual(afterTime.getTime());
    ...
    expect(
      auditLog?.timestamp,
      "AuditLog timestamp should be set",
    ).toBeDefined();
    expect(
      auditLog?.timestamp,
      "AuditLog timestamp should be a Date",
    ).toBeInstanceOf(Date);
  });
});
