...
// Global setup before all tests - only run if database is available
beforeAll(async () => {
...
// Global cleanup after all tests - only run if database is available
afterAll(async () => {
  if (!hasDatabaseUrl) return;
...
describe.skipIf(!hasDatabaseUrl)(
  "UserAdminService - CLIENT_OWNER Creation with Company",
  () => {
...
describe.skipIf(!hasDatabaseUrl)(
  "UserAdminService - CLIENT_OWNER Permission Scope Verification",
  () => {
